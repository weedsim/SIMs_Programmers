...
- [프로그래머스 바탕화면 정리 문제](https://school.programmers.co.kr/learn/courses/30/lessons/161990)

### 문제 풀이 방식

본래라면 그냥 vector를 그대로 이용하는 게 맞겠지만, 나는 제한이 세로 50에 가로 50이여서 어차피 용량 문제는 없을 것으로 보여 그냥 2차원 배열을 이용하였다.
먼저 주어지는 가로와 세로의 길이를 알 수 없기에 각 길이들을 구하고 2차원 배열에 각 `.`과 `#` 을 2차원 배열에 원하는 값으로 (나는 0으로 초기화 해주고 `.` 을 1로, `#` 을 2로 설정하였다.) 변경하고, 그 2차원배열을 탐색하여 X좌표의 최소와 최대, Y좌표의 최소와 최대를 구하여 각각 답으로 넣어주면 된다.
다만, 최대의 경우는 두 좌표다 +1을 해주어야한다.(이 부분은 문제에 설명이 상세히 되어있으니 문제를 꼼꼼하게 읽어보자.)